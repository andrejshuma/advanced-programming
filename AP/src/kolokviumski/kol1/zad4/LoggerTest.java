package kolokviumski.kol1.zad4;
...
class LogSystem<T extends ILog> {
        ...
        LogProcessor<T> firstLogProcessor = (logsList) -> (ArrayList<T>) logsList.stream().filter(log -> log.getType().equals("INFO")).collect(Collectors.toList());

        LogProcessor<T> secondLogProcessor = (logsList) -> (ArrayList<T>) logsList.stream().filter(log -> log.getMessage().length()<100).collect(Collectors.toList());

        LogProcessor<T> thirdLogProcessor = (logsList) -> (ArrayList<T>) logsList.stream().sorted(Comparator.comparing(ILog::getTimestamp)).collect(Collectors.toList());
