package kolokviumski.kol1.zad27;
...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    ArrayList<T> items;
    HashMap<String, TreeSet<T>> itemsMap;
    ...
    void addGenericItem (String category, T element){
        items.add(element);
        itemsMap.computeIfAbsent(category, k -> new TreeSet<>()).add(element);
    }

    Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to){
        return items.stream()
                .filter(item -> item.getTimestamp().isBefore(to) && item.getTimestamp().isAfter(from))
                .sorted(Comparator.reverseOrder())
                .collect(Collectors.toList());
    }
    Collection<T> itemsFromCategories (List<String> categories){
        List<T> result = new ArrayList<>();
        for (String category : categories.stream().distinct().collect(Collectors.toList())) {
            if (itemsMap.containsKey(category)) {
                result.addAll(itemsMap.get(category));
            }
        }
    ...
    public Map<String, Set<T>> byMonthAndDay(){
        TreeMap<String, Set<T>> ans = new TreeMap<>();
        for (T item : items){
            ...
            ans.computeIfAbsent(date, k-> new TreeSet<>(Comparator.reverseOrder())).add(item);
        }
        return ans;
    }
    public Map<Integer, Long> countByYear(){
        Map<Integer,Long> ans = new TreeMap<>();
        List<Integer> years = items.stream().map(item -> item.getTimestamp().getYear()).collect(Collectors.toList());

        for (Integer year : years) {
            long count = items.stream().filter(item -> item.getTimestamp().getYear() == year).count();
            ans.putIfAbsent(year,count);
        }
        return ans;
    }
}
