...
class EvaluatorBuilder<T extends Comparable<T>> {
    IEvaluator<T> build(String operator) {
        switch (operator) {
            case "<":
                return (left, right) -> ((Comparable<T>) left).compareTo((T) right) < 0;
            case ">":
                return (left, right) -> ((Comparable<T>) left).compareTo((T) right) > 0;
            case "==":
                return (left, right) -> ((Comparable<T>) left).compareTo((T) right) == 0;
...
class Evaluator {
    public static <T extends Comparable<T>> boolean evaluateExpression(T left, T right, String operator) {
        EvaluatorBuilder<T> builder = new EvaluatorBuilder<T>();
        IEvaluator<T> evaluator = builder.build(operator);
