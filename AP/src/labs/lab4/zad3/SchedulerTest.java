...
class Scheduler<T>{
    ...
    public T next(){
        ...
        return map.entrySet().stream()
                .filter(dateTEntry -> dateTEntry.getKey().after(now))
    ...
    public T last(){
        ...
        return map.entrySet().stream()
                .filter(dateTEntry -> dateTEntry.getKey().before(now))
                .max(Map.Entry.comparingByKey()).get().getValue();

    }

    public ArrayList<T> getAll(Date begin,Date end){
        return (ArrayList<T>) map.entrySet().stream()
                .filter(dateTEntry -> dateTEntry.getKey().after(begin)&&dateTEntry.getKey().before(end))
