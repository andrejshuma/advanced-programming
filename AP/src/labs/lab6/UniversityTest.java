...
class University {
    ...
    public Map<String, List<String>> getDepartmentToCourseNames() {
        return departments.stream().collect(Collectors.toMap(
                Department::getName,
                department -> department.getCourses().stream().map(Course::getName).collect(Collectors.toList())
        ));
        //TODO
    }
    ...
    public Optional<Department> getMostPopularDepartment() {
        return departments.stream()
                .max(Comparator.comparingInt(department -> department
                        .getCourses()
                        .stream()
                        .mapToInt(Course::getEnrolledStudents)
                        .sum()));
    ...
    public University mergeFourSmallestDepartments() {
        List<Department> filtered = departments.stream()
                .sorted(Comparator.comparingInt(d -> d.getCourses().stream().mapToInt(Course::getEnrolledStudents).sum()))
                .limit(4)
                .collect(Collectors.toList());

        Department dept=filtered.stream()
                .reduce((d1,d2)->{
                    String newName=d1.getName()+" & "+d2.getName();
                    List<Course>allCourses=Stream.concat(
                            d1.getCourses().stream(),
                            d2.getCourses().stream()
                    ).collect(Collectors.toList());
                    return new Department(newName,allCourses);
                }).orElse(null);

        List<Department>newDeps=new ArrayList<>(departments);
        newDeps.removeAll(filtered);
        newDeps.add(dept);
        return new University(newDeps);
        //TODO
    }
}
