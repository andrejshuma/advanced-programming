...
class  Timestamp<T> implements Comparable<Timestamp<?>>{
    ...
    public Timestamp(LocalDateTime dateTime, T object) {
...
class Scheduler <T>{
    List<Timestamp<T>> timestampList;
    T type;
    ...
    public Timestamp<T> next(){
        return timestampList.stream()
                .filter(ts -> ts.getTime().isAfter(LocalDateTime.now()))
    ...
    public Timestamp<T> last(){
        return timestampList.stream()
                .filter(ts -> ts.getTime().isBefore(LocalDateTime.now()))
    ...
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return timestampList.stream()
                .filter(ts -> !ts.getTime().isBefore(begin) && !ts.getTime().isAfter(end))
